...
package com.miband.app.preview
...
private fun PreviewSavedDevicesSection() {
    val controller = remember { ThemeController(ColorSchemeMode.Light) }
    val devices = listOf(
        com.miband.app.models.SavedDevice("1", "Mi Band 8", "AA:BB:CC:DD:EE:FF", "0123456789abcdef", connectType = "SPP"),
        com.miband.app.models.SavedDevice("2", "Xiaomi Watch S3", "11:22:33:44:55:66", "abcdef0123456789", connectType = "BLE"),
    )
    MiuixTheme(controller = controller) {
        Scaffold {
            Box(modifier = Modifier.padding(16.dp)) {
...
private fun PreviewAddDeviceScan() {
    ...
    val scannedDevices = listOf(
        com.miband.app.core.ScannedDevice("Mi Band 8", "AA:BB:CC:DD:EE:FF", -45),
        com.miband.app.core.ScannedDevice("Xiaomi Watch S3", "11:22:33:44:55:66", -60),
        com.miband.app.core.ScannedDevice("Redmi Band 2", "FF:EE:DD:CC:BB:AA", -72),
    )
    MiuixTheme(controller = controller) {
        Scaffold {
            OverlayBottomSheet(
...
private fun PreviewLogSectionContent() {
    val logs = listOf(
        LogEntry(System.currentTimeMillis(), "欢迎使用 BandBurg", LogType.INFO),
    )
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp)) {
            Text("操作日志", fontSize = 16.sp, fontWeight = FontWeight.Bold)
