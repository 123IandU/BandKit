...
impl L1CmdBuilder {
    pub fn new() -> Self {
        Self { cmd: None, config: HashMap::new() }
    }
    ...
    pub fn version(mut self, major: u8, minor: u8, patch: u8) -> Self {
        self.config.insert(config_type::CONFIG_TYPE_VERSION, vec![major, minor, patch]);
        self
    }

    pub fn mps(mut self, mps: u16) -> Self {
        self.config.insert(config_type::CONFIG_TYPE_MPS, mps.to_le_bytes().to_vec());
        self
    }

    pub fn tx_win(mut self, win: u16) -> Self {
        self.config.insert(config_type::CONFIG_TYPE_TX_WIN, win.to_le_bytes().to_vec());
        self
    }

    pub fn send_timeout(mut self, timeout_ms: u16) -> Self {
        self.config.insert(config_type::CONFIG_TYPE_SEND_TIMEOUT, timeout_ms.to_le_bytes().to_vec());
        self
    }

    pub fn device_type(mut self, dev_type: u8) -> Self {
        self.config.insert(config_type::CONFIG_TYPE_DEVICE_TYPE, vec![dev_type]);
        self
    }

    pub fn device_name<S: AsRef<[u8]>>(mut self, name: S) -> Self {
        self.config.insert(config_type::CONFIG_TYPE_DEVICE_NAME, name.as_ref().to_vec());
        self
    }

    pub fn os_version(mut self, major: u8, minor: u8, patch: u8) -> Self {
        self.config.insert(config_type::CONFIG_TYPE_OS_VERSION, vec![major, minor, patch]);
        self
    }

    pub fn build(self) -> Option<L1CmdPacket> {
        Some(L1CmdPacket { cmd: self.cmd?, config: self.config })
    }
}
...
impl L1CmdPacket {
    ...
    pub fn from_payload_bytes(payload: &[u8]) -> Option<Self> {
        ...
        while i + 3 <= payload.len() {
            ...
            i += 3;
            if i + value_size > payload.len() { break; }
    ...
    pub fn get_version(&self) -> Option<(u8, u8, u8)> {
        self.config.get(&config_type::CONFIG_TYPE_VERSION).and_then(|v| {
            if v.len() == 3 { Some((v[0], v[1], v[2])) } else { None }
        })
    }

    pub fn get_mps(&self) -> Option<u16> {
        self.config.get(&config_type::CONFIG_TYPE_MPS).and_then(|v| {
            if v.len() == 2 { Some(u16::from_le_bytes([v[0], v[1]])) } else { None }
        })
    }

    pub fn get_tx_win(&self) -> Option<u16> {
        self.config.get(&config_type::CONFIG_TYPE_TX_WIN).and_then(|v| {
            if v.len() == 2 { Some(u16::from_le_bytes([v[0], v[1]])) } else { None }
        })
    }

    pub fn get_send_timeout(&self) -> Option<u16> {
        self.config.get(&config_type::CONFIG_TYPE_SEND_TIMEOUT).and_then(|v| {
            if v.len() == 2 { Some(u16::from_le_bytes([v[0], v[1]])) } else { None }
        })
    }

    pub fn get_device_type(&self) -> Option<u8> {
        self.config.get(&config_type::CONFIG_TYPE_DEVICE_TYPE).and_then(|v| v.get(0).copied())
    }

    pub fn get_device_name(&self) -> Option<String> {
        self.config.get(&config_type::CONFIG_TYPE_DEVICE_NAME).and_then(|v| String::from_utf8(v.clone()).ok())
    }

    pub fn get_os_version(&self) -> Option<(u8, u8, u8)> {
        self.config.get(&config_type::CONFIG_TYPE_OS_VERSION).and_then(|v| {
            if v.len() == 3 { Some((v[0], v[1], v[2])) } else { None }
        })
    }
}
